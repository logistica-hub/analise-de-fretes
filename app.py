...
if menu == "📊 Dashboard":
    ...
    if res.data:
        ...
        if all_dfs:
            ...
            # --- SEÇÃO DE FILTROS SOLICITADA ---
            st.subheader("🎯 Filtros do Dashboard")
            c_f1, c_f2 = st.columns(2)
            
            lista_ufs = sorted(df_total['UF'].unique().tolist()) if 'UF' in df_total.columns else []
            sel_ufs = c_f1.multiselect("Filtrar por UF", lista_ufs, default=lista_ufs)
            
            cols_frete = [c for c in df_total.columns if c.startswith("TOTAL_")]
            lista_transp = [c.replace("TOTAL_", "") for c in cols_frete]
            sel_transp = c_f2.multiselect("Filtrar Transportadoras", lista_transp, default=lista_transp)
            
            # Aplicando Filtros
            df_filt = df_total.copy()
            if sel_ufs:
                df_filt = df_filt[df_filt['UF'].isin(sel_ufs)]
            
            cols_f_sel = [f"TOTAL_{t}" for t in sel_transp]
            
            if not df_filt.empty and cols_f_sel:
                # Métricas
                m1, m2, m3 = st.columns(3)
                m1.metric("Notas Filtradas", len(df_filt))
                gasto_total = df_filt[cols_f_sel].sum().sum()
                m2.metric("Gasto Total (Filtro)", f"R$ {gasto_total:,.2f}")
                melhor = df_filt[cols_f_sel].sum().idxmin().replace("TOTAL_", "")
                m3.metric("Melhor Opção no Filtro", melhor)

                st.divider()
                st.subheader("🌍 Visão por UF")
                resumo_uf = df_filt.groupby('UF')[cols_f_sel].sum()
        ...
        if df_t is not None and df_a is not None:
            ...
            for i in range(int(n_f)):
                ...
                faixas.append({
                    ...
                    "col": r[2].selectbox("Coluna", cols_t, index=cols_t.index(f_i.get('col')) if f_i.get('col') in cols_t else 0, key=f"co{i}")
                })
            ...
            if st.button("💾 Salvar Transportadora"):
                ...
                payload = {"nome": nome_t, "tabela_json": df_t.replace([np.inf, -np.inf], 0).fillna(0).to_dict(orient='records'), "cidades_json": df_a.fillna("").to_dict(orient='records'), "mapeamento_json": mapa_f}
        ...
        if selecionadas and st.button("🚀 Calcular"):
            with st.spinner("Processando..."):
                ...
                for t_nome in selecionadas:
                    ...
                    sig_match = pd.Series(cid_notas).map(dic_abr).fillna("N/D").values
                    
                    # CORREÇÃO CRÍTICA: Indexar a tabela SEM normalizar a sigla novamente, 
                    # para dar match exato com o que saiu do dicionário acima.
                    ...
                    for f in m['faixas']:
                        if f['col'] in df_tab_idx.columns:
                            ...
                            vals_m = np.array([float(dict_p.get(s, 0)) for s in sig_match])
                    ...
                    if m.get('kg_extra') in df_tab_idx.columns:
                        ...
                        if mask_e.any():
                            d_b = df_tab_idx[m['faixas'][-1]['col']].to_dict()
                            d_e = df_tab_idx[m['kg_extra']].to_dict()
                            v_b = np.array([float(d_b.get(s, 0)) for s in sig_match])
                            v_e = np.array([float(d_e.get(s, 0)) for s in sig_match])
                            f_peso[mask_e] = v_b[mask_e] + ((pesos_notas[mask_e] - u_max) * v_e[mask_e])

                    def gv(tx_n):
                        c = m['taxas'].get(tx_n, "Não mapear")
                        if c in df_tab_idx.columns:
                            d = df_tab_idx[c].to_dict()
                            return np.array([float(d.get(s, 0)) for s in sig_match])
                    ...
                    if m['tab_uf'] in df_tab_idx.columns:
                        ...
                        df_final['UF'] = [d_uf.get(s, "ND") for s in sig_match]
