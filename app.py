...
with st.sidebar:
    ...
    if st.session_state.logo_data:
        ...
        if up_logo:
            ...
            st.rerun()
    st.divider()
    menu = st.radio("Navegação", ["📊 Dashboard", "🚛 Cadastro", "💰 Comparativo"])
            ...
            if all_dfs:
                ...
                if not df_filt.empty and cols_sel:
                    ...
                    if col_uf:
                        ...
                        def highlight_min(s):
                            return ['background-color: #ecfdf5; color: #065f46; font-weight: bold' if v == s.min() else '' for v in s]
                        st.dataframe(df_uf.style.apply(highlight_min, axis=1).format(format_brl), use_container_width=True)
    except:
        st.info("Aguardando dados ou erro na conexão com banco.")
...
# --- CADASTRO ---
    ...
    with st.expander("📝 Nova / Editar Transportadora", expanded=st.session_state.get('edit_id') is not None):
        ...
        f_abr = c2.file_uploader("📂 Cidades/Siglas", type=["xlsx"], key=f"a_{st.session_state.form_reset_key}")
        ...
        if df_t is not None and df_a is not None:
            ...
            for i in range(int(n_f)):
                ...
                faixas.append({
                    ...
                    "col": r[2].selectbox(f"Coluna faixa {i+1}", cols_t, index=cols_t.index(f_i.get('col')) if f_i.get('col') in cols_t else 0, key=f"co{i}")
                })
...
# --- COMPARATIVO ---
            ...
            with st.spinner("Processando..."):
                ...
                for t_nome in selecionadas:
                    ...
                    for faixa in m['faixas']:
                        v_f = get_v(faixa['col']); mask = (pesos_notas <= faixa['max']) & (f_peso == 0.0)
                    ...
                    tas = get_v(m['taxas'].get("TAS")); ctrc = get_v(m['taxas'].get("CTRC"))
                ...
                try:
                    ...
                    st.success("Cálculo realizado!"); st.rerun()
                except Exception as e:
                    st.error("Erro ao salvar no banco. O arquivo pode ser muito grande para o limite do Supabase.")
    ...
    if res_h.data:
        for t_ref, g in pd.DataFrame(res_h.data).groupby("data_hora", sort=False):
            with st.expander(f"📦 {t_ref}"):
                if st.button("🗑️ Excluir", key=f"del_{t_ref}"):
